...
| F5 | Structured Data | P2 | Events, Blogs | Phase 4C visible breadcrumbs & BreadcrumbList JSON-LD implemented on public detail pages. | Missed opportunity for eligible rich results. | Inject `WebSite`, `Organization`, `BlogPosting`, and `BreadcrumbList` JSON-LD; defer `Event` schema. | Med | High | None | WebSite, Organization, BlogPosting, and BreadcrumbList render truthful JSON-LD in SSR. Event schema deferred. |
