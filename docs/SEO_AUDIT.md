...
| F5 | Structured Data | P2 | Events, Blogs | Phase 4A local foundation implemented. | Missed opportunity for eligible rich results. | Inject `WebSite`, `Organization`, and `BlogPosting` JSON-LD; defer `Event` and `BreadcrumbList`. | Med | High | None | WebSite, Organization, and BlogPosting render truthful JSON-LD in SSR. Event & BreadcrumbList deferred. |
